...
async def main_menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == 'main_menu':
        ...
        await handle_status_update(update, context)
...
async def buy_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query:
        # For callback queries, always send a new message to avoid issues with photo captions
        try:
...
# Admin: Update Order Status - Show current status with update options
async def admin_status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text(
            "⚠️ Usage: `/status <order_id>`\n\n"
            "This will show the current order status and allow you to update it.",
            parse_mode='Markdown'
        )
        return
    ...
    product = conn.execute("SELECT name FROM products WHERE code = ?", (order['product_code'],)).fetchone()
    user = conn.execute("SELECT username FROM users WHERE user_id = ?", (order['user_id'],)).fetchone()
    conn.close()
    ...
    all_statuses = ['Pending', 'Accepted', 'Out for Delivery', 'Delivered']
    current_status = order['status']
    
    text = (
        f"📋 **Order Status Management**\n"
        f"━━━━━━━━━━━━━━━━━━━━\n\n"
        f"🆔 Order ID: `{order_id}`\n"
        f"👤 User: @{username} ({order['user_id']})\n"
        f"📦 Product: {product_name}\n"
        f"💰 Price: ₹{order['price_at_time']} {order['currency']}\n"
        f"📍 Address: {order['delivery_address']}\n"
        f"💳 Payment: {order['payment_method']}\n"
        f"📅 Date: {order['timestamp']}\n\n"
        f"**Current Status:** {status_emoji.get(current_status, '📋')} **{current_status}**\n\n"
        f"👇 **Tap to update status:**"
    )
    
    # Create buttons for each status (highlight current)
    buttons = []
    for status in all_statuses:
        emoji = status_emoji.get(status, '📋')
        if status == current_status:
            btn_text = f"✓ {emoji} {status} (Current)"
        else:
            btn_text = f"{emoji} {status}"
        # Replace spaces with underscores for callback_data
        status_code = status.replace(' ', '_')
        buttons.append([InlineKeyboardButton(btn_text, callback_data=f'set_status_{order_id}_{status_code}')])
    
    # Add cancel option if not already cancelled
    if current_status != 'Cancelled':
        buttons.append([InlineKeyboardButton("❌ Cancel Order", callback_data=f'set_status_{order_id}_Cancelled')])
...
async def handle_status_update(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Parse callback data: set_status_{order_id}_{status_code}
    # Format: set_status_ORD-XXXXXX_Status_Name (underscores replace spaces)
    data = query.data
    # Remove 'set_status_' prefix
    rest = data[len('set_status_'):]
    # Order ID format: ORD-XXXXXX (contains hyphen, not underscore)
    # Split at first underscore after the order_id
    first_underscore = rest.find('_')
    
    # Validate that underscore was found
    if first_underscore == -1:
        await query.edit_message_text("❌ Invalid status update format.")
        return
    
    order_id = rest[:first_underscore]
    status_code = rest[first_underscore + 1:]
    # Convert underscores back to spaces for the status
    new_status = status_code.replace('_', ' ')
    ...
    if old_status == new_status:
        await query.answer("Status is already set to this value.", show_alert=True)
        conn.close()
        return
