AAHyyRt0P24dGWa2mZ5AXdTeNBqeI-eLmCo'yRt0P24dGWa2mZ5AXdTeNBqeI-eLmCo'rt logging
import sqlite3
import uuid
...
TOKEN = '8200854859:AAGqwyeAXRzTmwervJuFN6GVJK_wuRwj5rg'  # <--- REPLACE WITH YOUR BOT TOKEN
ADMIN_ID = 6083286836           # <--- REPLACE WITH YOUR TELEGRAM ID
SUPPORT_USERNAME = "@mukeshgkv"
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_admin:
        text += (
            ...
            "• `/status <order_id>` - Interactively update an order's status\n\n"
            "**User Management:**\n"
...
async def main_menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # If the bot is in a conversation, don't let this handler interfere.
    if context.user_data.get('in_conversation'):
        await query.answer("Please complete the current action first.")
        return
...
async def buy_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not product['availability']:
        ...
        conn.close()
        context.user_data['in_conversation_data'] = False
        return ConversationHandler.END
    ...
    if query:
        # Check if the original message had a caption (was a photo)
        if query.message.caption:
            await query.edit_message_caption(caption=text, parse_mode='Markdown', reply_markup=InlineKeyboardMarkup(buttons))
        else:
            await query.edit_message_text(text, parse_mode='Markdown', reply_markup=InlineKeyboardMarkup(buttons))
    else:
...
async def buy_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    buttons.append([InlineKeyboardButton("🔙 Cancel", callback_data='buy_cancel')])
...
async def buy_address_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'addr_saved':
        ...
        await query.edit_message_text("📝 Please type your full delivery address now (or /cancel):")
...
async def update_address_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    await query.edit_message_text("📝 Please enter your new delivery address (or /cancel):")
...
async def show_settings(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    query = update.callback_query
    if query and query.data.startswith('set_curr_'):
...
async def show_orders(update: Update, context: ContextTypes.DEFAULT_TYPE, active_only=True):
    ...
    status_emoji = {
        'Pending': '⏳', 'Accepted': '✅', 'Out for Delivery': '🚚', 'Delivered': '📦', 'Cancelled': '❌'
    }
    ...
    for o in orders:
        ...
        text += (
            ...
            f"💰 Price: {format_price(o['price_at_time'], o['currency'])}\n"
            f"📅 Date: {o['timestamp']}\n"
        )
...
async def admin_list_items(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not products: text += "No active products."
    ...
    buttons = []
    nav_row = []
    if page > 0: nav_row.append(InlineKeyboardButton("⬅️ Previous", callback_data=f'adm_list_items_{page-1}'))
    if (offset + ITEMS_PER_PAGE) < total: nav_row.append(InlineKeyboardButton("Next ➡️", callback_data=f'adm_list_items_{page+1}'))
...
async def admin_pending_orders(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = get_db_connection()
    orders = conn.execute("SELECT * FROM orders WHERE status = 'Pending' ORDER BY timestamp DESC LIMIT ? OFFSET ?", (ITEMS_PER_PAGE, offset)).fetchall()
    total = conn.execute("SELECT COUNT(*) FROM orders WHERE status = 'Pending'").fetchone()[0]
    
    text = "⏳ **Pending Orders**\n━━━━━━━━━━━━━━━━━━━━\n\n"
    if not orders: text += "No pending orders."
    
    for o in orders:
        user = conn.execute("SELECT username FROM users WHERE user_id = ?", (o['user_id'],)).fetchone()
        product = conn.execute("SELECT name FROM products WHERE code = ?", (o['product_code'],)).fetchone()
        text += (f"🆔 Order: `{o['order_id']}`\n"
    ...
    buttons = []
    nav_row = []
    if page > 0: nav_row.append(InlineKeyboardButton("⬅️ Previous", callback_data=f'adm_pending_{page-1}'))
    if (offset + ITEMS_PER_PAGE) < total: nav_row.append(InlineKeyboardButton("Next ➡️", callback_data=f'adm_pending_{page+1}'))
    if nav_row: buttons.append(nav_row)
    buttons.append([InlineKeyboardButton("🔙 Back to Admin Panel", callback_data='admin_panel')])
...
async def admin_all_orders(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = get_db_connection()
    orders = conn.execute("SELECT * FROM orders ORDER BY timestamp DESC LIMIT ? OFFSET ?", (ITEMS_PER_PAGE, offset)).fetchall()
    total = conn.execute("SELECT COUNT(*) FROM orders").fetchone()[0]
    
    text = "📦 **All Orders**\n━━━━━━━━━━━━━━━━━━━━\n\n"
    if not orders: text += "No orders yet."
    
    status_emoji = {'Pending': '⏳', 'Accepted': '✅', 'Out for Delivery': '🚚', 'Delivered': '📦', 'Cancelled': '❌'}
    ...
    buttons = []
    nav_row = []
    if page > 0: nav_row.append(InlineKeyboardButton("⬅️ Previous", callback_data=f'adm_all_orders_{page-1}'))
    if (offset + ITEMS_PER_PAGE) < total: nav_row.append(InlineKeyboardButton("Next ➡️", callback_data=f'adm_all_orders_{page+1}'))
    if nav_row: buttons.append(nav_row)
    buttons.append([InlineKeyboardButton("🔙 Back to Admin Panel", callback_data='admin_panel')])
...
async def add_item_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.edit_message_text("📝 Enter Product Name (or /cancel):")
    return ADD_NAME
...
async def add_price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if orig <= price:
            await update.message.reply_text("Discounted price must be less than the original price. Please try again.")
            return ADD_PRICE
...
async def add_img(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await show_product_summary(update, context)
    return ConversationHandler.END
...
async def show_product_summary(update: Update, context: ContextTypes.DEFAULT_TYPE):
    item = context.user_data['new_item']
    summary = (
        f"📋 **Product Summary**\n━━━━━━━━━━━━━━━━━━━━\n\n"
        f"**Name:** {item['name']}\n"
        ...
        f"**Image:** {'✅ Uploaded' if item.get('image_id') else '❌ No image'}\n\n"
...
async def admin_handle_order(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == 'acc':
        ...
        if query.message.caption: await query.edit_message_caption(caption=text)
        else: await query.edit_message_text(text)
    
    elif action == 'can':
        context.user_data['cancel_order_id'] = order_id
        await query.message.reply_text(f"📝 Please provide a reason for cancelling order `{order_id}` (or /cancel):", parse_mode='Markdown')
...
async def admin_delist_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (f"🗑️ **Confirm Delisting**\n\n"
            f"Product: {product['name']}\nCode: `{product['code']}`\nPrice: ₹{product['price']}\n\n"
            f"Are you sure you want to remove this item?")
    buttons = [[InlineKeyboardButton("✅ Confirm Delist", callback_data=f'confirm_delist_{code}')],
               [InlineKeyboardButton("❌ Cancel", callback_data='admin_panel')]]
    conn.close()
    await update.message.reply_text(text, parse_mode='Markdown', reply_markup=InlineKeyboardMarkup(buttons))
...
async def admin_accept_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(chat_id=order['user_id'], text=f"✅ **Order Accepted!**\n\nYour order `{order_id}` has been accepted!", parse_mode='Markdown')
...
async def admin_cancel_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"📝 Please provide a reason for cancelling order `{order_id}`:", parse_mode='Markdown')
    return CANCEL_REASON
...
async def cancel_reason_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if order:
        conn.execute("UPDATE orders SET status = 'Cancelled', rejection_reason = ? WHERE order_id = ?", (reason, order_id))
        conn.commit()
        await context.bot.send_message(chat_id=order['user_id'], text=f"❌ **Order Cancelled**\n\nOrder ID: `{order_id}`\nReason: {reason}", parse_mode='Markdown')
        await update.message.reply_text(f"✅ Order `{order_id}` cancelled.", parse_mode='Markdown')
    conn.close()
    context.user_data.pop('cancel_order_id', None)
    return ConversationHandler.END

# ---- NEW /status command logic ----
async def admin_status_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ADMIN_ID: return
    if not context.args:
        await update.message.reply_text("⚠️ Usage: `/status <order_id>`", parse_mode='Markdown')
        return ConversationHandler.END
    ...
    next_statuses = status_options.get(current_status, [])
    
    text = (f"⚙️ **Update Status for Order `{order_id}`**\n\n"
            f"**Current Status:** {current_status}\n\n"
            f"Select the new status for this order:")
    
    buttons = []
    if next_statuses:
        for status in next_statuses:
            buttons.append([InlineKeyboardButton(f"Set to: {status}", callback_data=f"set_status_{status}")])
    else:
        text += "\n\nThis order cannot be updated further."

    buttons.append([InlineKeyboardButton("Done", callback_data="set_status_done")])
    await update.message.reply_text(text, parse_mode='Markdown', reply_markup=InlineKeyboardMarkup(buttons))
    return ADMIN_UPDATE_STATUS

async def admin_status_update(update: Update, context: ContextTypes.DEFAULT_Type):
    query = update.callback_query
    await query.answer()
    ...
    if data == "set_status_done":
        await query.edit_message_text(f"✅ Finished updating order `{order_id}`.")
    ...
    await context.bot.send_message(
        ...
        text=(f"{status_emoji.get(new_status, '📋')} **Order Status Updated**\n\n"
              f"Order ID: `{order_id}`\n"
              f"New Status: **{new_status}**"),
        parse_mode='Markdown'
    )
    await query.edit_message_text(f"✅ Order `{order_id}` status updated to **{new_status}**.", parse_mode='Markdown')
    context.user_data.pop('status_order_id', None)
    return ConversationHandler.END
...
async def admin_user_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    total_orders = conn.execute("SELECT COUNT(*) FROM orders WHERE user_id = ?", (user_id,)).fetchone()[0]
    pending_orders = conn.execute("SELECT COUNT(*) FROM orders WHERE user_id = ? AND status NOT IN ('Delivered', 'Cancelled')", (user_id,)).fetchone()[0]
    conn.close()
    ...
    buttons = []
    if user['is_banned']: buttons.append([InlineKeyboardButton("✅ Unban User", callback_data=f'unban_{user_id}')])
    else: buttons.append([InlineKeyboardButton("🚫 Ban User", callback_data=f'ban_{user_id}')])
    buttons.append([InlineKeyboardButton("🔙 Back", callback_data='admin_panel')])
    await update.message.reply_text(text, parse_mode='Markdown', reply_markup=InlineKeyboardMarkup(buttons))

# Admin: Ban/Unban User Commands
...
# Callback handlers for ban/unban buttons
...
def main():
    ...
    conv_handler_cancel = CommandHandler('cancel', cancel)
    ...
    conv_buy = ConversationHandler(
        ...
        states={
            ...
            BUY_PAYMENT: [CallbackQueryHandler(buy_payment, pattern='^pay_|^buy_cancel$')]
        },
        fallbacks=[conv_handler_cancel]
    )
    ...
    conv_cancel_order = ConversationHandler(
        ...
        states={CANCEL_REASON: [MessageHandler(filters.TEXT & ~filters.COMMAND, cancel_reason_handler)]},
        fallbacks=[conv_handler_cancel]
    )

    conv_update_address = ConversationHandler(
        entry_points=[CallbackQueryHandler(update_address_start, pattern='^update_address$')],
        states={PROFILE_ADDRESS: [MessageHandler(filters.TEXT & ~filters.COMMAND, update_address_save)]},
        fallbacks=[conv_handler_cancel]
    )
    
    conv_admin_status = ConversationHandler(
        entry_points=[CommandHandler('status', admin_status_start)],
        states={ADMIN_UPDATE_STATUS: [CallbackQueryHandler(admin_status_update, pattern='^set_status_')]},
        fallbacks=[conv_handler_cancel]
    )

    # --- HANDLER REGISTRATION ---
    # IMPORTANT: ConversationHandlers must be added BEFORE other handlers that might catch the same updates.
    ...
    application.add_handler(conv_admin_status)
    ...
    # Main Callback Handler (handles all non-conversation callback queries)
